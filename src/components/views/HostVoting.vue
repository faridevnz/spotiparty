...
export default {
   ...
   watch: {
      party_mode(oldValue, newValue) {
         if (oldValue.battle_songs != newValue.battl_songs) {
            this.assignBattleSongs()
         }
      }
   },
   methods: {
      ...
      changeTab(index) {
         if (index == 0) {
            this.$router.push({ name: 'HostPartyTracks' })
         } else {
            this.$router.push({ name: 'HostProposedTracks' })
         }
      }
   },
   created() {
      if (this.$router.currentRoute.name != 'HostPartyTracks') {
         this.$router.push({ name: 'HostPartyTracks' })
      }
   }
}
